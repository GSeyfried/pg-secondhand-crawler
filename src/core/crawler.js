...
export async function runCrawler({config,adapter,storage,logger}) {
  const summary=emptySummary();await storage.init(adapter.name);const cacheDir=path.join(storage.root,'cache');const http=new HttpClient({...config,cacheDir,logger});
  ...
  for(let at=0;at<urls.length;at+=config.batchSize){const batchNo=initialBatches+Math.floor(at/config.batchSize)+1;const batch=urls.slice(at,at+config.batchSize);const staged=[];
    ...
    await storage.saveStaging(adapter.name,batchNo,staged);const historyUpdate=updateListingHistory(histories,staged,{observedAt:new Date().toISOString(),completeSnapshot:false,source:adapter.name});histories=historyUpdate.histories;summary.newListingsFound+=historyUpdate.report.newListings.length;summary.priceDropsDetected+=historyUpdate.report.priceDrops.length;await storage.saveDatabase(db);await storage.saveHistory(histories);checkpoint.completedUrls=[...completed];checkpoint.nextOffset=Math.max(checkpoint.nextOffset??0,...batch.map(url=>(discoveryIndex.get(url)??-1)+1));checkpoint.batchesCompleted=batchNo;checkpoint.lastDiscoveryCount=summary.listingUrlsDiscovered;await storage.saveCheckpoint(adapter.name,checkpoint);logger.info('Batch complete',{batchNumber:batchNo,attempted:batch.length,staged:staged.length,failures:settled.filter(x=>x.status==='rejected').length});
  }
  const kitComparisons=buildKitComparisons(db);await storage.saveComparisons(kitComparisons);summary.fullKitsAvailable=kitComparisons.summary.fullKits;summary.assembledKitCandidates=kitComparisons.summary.assembledCandidates;return summary;
}
